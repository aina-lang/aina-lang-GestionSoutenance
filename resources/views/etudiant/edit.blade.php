<x-app-layout>
    <div class="overflow-x-auto  sm:ml-64 mb-10">
     <div class="logo mb-10">
         <h1 class="text-bold text-xl text-indigo-500  mb-9 xl:mb-0 xl:mt-2">Modification de l'etudiant N:{{ $etudiant->matricule }}</h1>
     </div>
     <form action="{{ route('etudiant.update', $etudiant->matricule) }}" method="POST" class="xl:w-3/5  mx-auto p-4  bg-white shadow-md rounded-md">
        @csrf
        @method('PATCH')
             ...
             <div class="w-full">
                 <div class="mb-4 w-full">
                     ...
                     <input type="text" id="matricule" name="matricule"
                         class="w-full px-3 py-2 border rounded-md outline-none focus:ring-2 focus:ring-blue-500" value="{{ $etudiant->matricule }}"
                         required>
                 </div>
                 <div class="mb-4">
                     <label for="nom" class="block text-gray-700 font-bold mb-2">Nom</label>
                     <input type="text" id="nom" name="nom"
                         class="w-full px-3 py-2 border rounded-md outline-none focus:ring-2 focus:ring-blue-500" value="{{ $etudiant->nom }}"
                         required>
                 </div>
                 <div class="mb-4">
                     <label for="prenoms" class="block text-gray-700 font-bold mb-2">Prénoms</label>
                     <input type="text" id="prenom" name="prenom"
                         class="w-full px-3 py-2 border rounded-md outline-none focus:ring-2 focus:ring-blue-500"   value="{{ $etudiant->prenom }}"
                         required>
                 </div>
             </div>
             <div class="w-full">
